...
namespace BpmOnlineConfig
{
    public partial class BpmonlineConfiguration : UserControl
    {
        ...
        public void Initialize(string siteName, string virtualPath)
        {
	        ...
	        SubscribeOnControlChange(Controls);
        }

	    private void SubscribeOnControlChange(ControlCollection controls) {
			foreach (Control control in controls) {
				if (control is Label) {
					continue;
				}
				if (control is GroupBox) {
					SubscribeOnControlChange(((GroupBox)control).Controls);
					continue;
				}
				if (control is CheckBox) {
					((CheckBox)control).CheckedChanged += OnFormControlChanged;
				}
				if (control is NumericUpDown) {
					((NumericUpDown)control).ValueChanged += OnFormControlChanged;
				} else {
					control.TextChanged += new EventHandler(OnFormControlChanged);
				}
			}
	    }
